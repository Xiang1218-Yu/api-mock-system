...
func (h *Handler) Serve(c *gin.Context) {
	...
	c.JSON(status, gin.H{"result": merged, "downstream_status": results})
	h.recordCall(projectID, agg.ID, c.Request.Method, path, status, int(time.Since(start)/time.Millisecond))
}
